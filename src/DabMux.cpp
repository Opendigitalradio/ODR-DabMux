...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        if (argc == 2)
        { // Assume the only argument is a config file
            conf_file = argv[1];
        ...
        for (auto ptree_pair : pt_outputs)
        {
            ...
            if (outputuid == "edi")
            {
                ...
                for (auto pt_edi_dest : pt_edi.get_child("destinations"))
                {
                    ...
                    else if (proto == "ts")
                    {
                        ...
                        dest->ts_id = pt_edi_dest.second.get<unsigned int>("ts_id");
                        ...
                        dest->service_provider_name = pt_edi_dest.second.get<string>("service_provider_name");
                        ...
                        dest->output_source_address = pt_edi_dest.second.get<string>("output_source_address");
                        dest->output_ttl = pt_edi_dest.second.get<unsigned int>("output_ttl");

                        if (dest->output == "srt")
                        {
                            dest->output_srt_passphrase = pt_edi_dest.second.get<string>("output_srt_passphrase");
                        }
            ...
            else if (outputuid == "zeromq")
            {
                ...
                if (not output)
                {
                    etiLog.level(error) << "Unable to init zeromq output " << endpoint;
                    return -1;
                }

                if (output->Open(location) == -1)
                {
                    etiLog.level(error) << "Unable to open zeromq output " << endpoint;
                    return -1;
                }
            ...
            else
            {
                ...
                if (not output)
                {
                    etiLog.level(error) << "Unable to init output " << uri;
                    return -1;
                }

                if (output->Open(location) == -1)
                {
                    etiLog.level(error) << "Unable to open output " << uri;
                    return -1;
                }
        ...
        for (currentFrame = 0; running; currentFrame++)
        {
            ...
            if (currentFrame % 10 == 0)
            {
                ...
                {
                    etiLog.level(warn) << "Detected Management Server fault, restarting it";
                    mgmt_server.restart();
                }
